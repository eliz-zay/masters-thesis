  ...
  private:
    ...
    PreservedAnalyses applyPass(Function &F) const override {
      ...
      for (auto &block : F) {
        ...
        if (caseVar == nullptr) {
          errs() << "[" << BogusSwitchPass::annotationName << "] Unable to identify switch variable\n";
          continue;
        }

        // Check if switch was generated by control-flow flattening pass,
        // meaning it is safe to remap `caseVar` assignments to indexes of duplicated cases
        bool isSwitchFlattened = this->checkIfSwitchFlattened(F, switchInst);
