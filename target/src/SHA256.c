...
static int calcFileSize();
static void endianCheckPrint();
static _Bool endianCheck();
static int fillMessageBlock();
static void calculateHash(FILE *file);
static int nextMessageBlock(FILE *file, union messageBlock *msgBlock, enum status *state, __uint64_t *numBits);
